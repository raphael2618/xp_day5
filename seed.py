...
#
# for type in VehicleType.objects.filter(name = 'Bike'): rate = 5 for size in VehicleSize.objects.all():
# RentalRate.objects.create(daily_rate= rate, vehicle_type=VehicleType.objects.get(name= type.name),
# vehicle_size=VehicleSize.objects.get(name= size.name)) rate +=2
#
